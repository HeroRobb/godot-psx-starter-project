...
signal body_entered(body: CollisionObject2D)
signal body_exited(body: CollisionObject2D)
...
@export var override_projectile_motion: Projectile2D.MOTION_TYPES = Projectile2D.MOTION_TYPES.LINEAR
@export_category("Projectile modifiers")
@export var can_damage: HitboxComponent2D.HURTBOX_TYPES = HitboxComponent2D.HURTBOX_TYPES.FRIENDLY
...
var _physics_parent: PhysicsBody2D
var _on_cooldown: bool = false
var _previous_collider: CollisionObject2D
...
func fire() -> void:
	...
	var new_projectile: Projectile2D = _projectile_scene.instantiate()
#	new_projectile.forward_direction =  Vector2.RIGHT.rotated(Vector3.UP, global_rotation.y)
	
	if _physics_parent:
		new_projectile.add_collision_exception_with(_physics_parent)
	add_exception(new_projectile)
	new_projectile.can_damage = can_damage
	SignalManager.instance_spawned.emit(new_projectile)
	new_projectile.global_position = global_position + target_position
	new_projectile.forward_direction = target_position
