...
@export var can_damage: HitboxComponent3D.HURTBOX_TYPES = HitboxComponent3D.HURTBOX_TYPES.FRIENDLY
...
@export var destroy_offscreen: bool = true
...
var _previous_wave_tangent: Vector3 = Vector3.ZERO

@onready var _movement_component: MovementComponent3D = $MovementComponent3D
@onready var _mesh_component: MeshComponent = $MeshComponent
@onready var _hitbox: HitboxComponent3D = $HitboxComponent3D
...
func _physics_process(delta: float) -> void:
	...
	for collider_id in collision.get_collision_count():
		var collider: Object = collision.get_collider(collider_id)
		collider
...
func _get_tangent_wave_direction(delta: float) -> void:
	...
	var tangent_normalized: Vector3 = tangent_vector.normalized()
	var direction_perpendicular: Vector3 = _get_perpendicular_vector(forward_direction.normalized())
	...
	if _angle_is_relavent(angle):
		tangent_vector = tangent_vector.rotated(Vector3.UP, angle)
	var new_modified_tangent: Vector3 = tangent_vector - _previous_wave_tangent
...
func _spawn_explosion() -> void:
	...
	var explosion_instance: Node3D = explosion_packed_scene.instantiate()
	explosion_instance.global_position = global_position
	explosion_instance.global_rotation = global_rotation
	get_parent().add_child(explosion_instance)


func _get_perpendicular_vector(from_vector: Vector3) -> Vector3:
